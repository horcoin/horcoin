...
static const int64_t MAX_MONEY = 10000000 * COIN; // max coin
...
public:
    ...
    {
        // horizoncoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // horizoncoin: get transaction coin age
...
public:
    ...
    // horizoncoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // horizoncoin: two types of block: proof-of-work or proof-of-stake
    ...
    // horizoncoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // horizoncoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // horizoncoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // horizoncoin: block index flags
    enum  
    {
