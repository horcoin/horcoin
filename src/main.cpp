...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HorizonCoin: starting difficulty is 1 / 2^12
...
{
    // HorizonCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60 * 60; // HorizonCoin: 1 hour
static const int64 nTargetSpacing = 3600; // HorizonCoin: 60 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HorizonCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 42; // HorizonCoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // HorizonCoin: 1 day
...
{
    // HorizonCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
        ...
        block.nVersion = 1;
        block.nTime    = 1403704800;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 325433;

        if (fTestNet)
        {
            block.nTime    = 1403196482;
            block.nNonce   = 325433;
        }
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // HorizonCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HorizonCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HorizonCoinMiner : ProcessBlock, block not accepted");
    }
